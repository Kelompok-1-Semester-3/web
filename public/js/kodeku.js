        ...
        if (e.keyCode == 13) {
            ...
            if (keyword.length == 0) {
                ...
                $.ajax({
                    ...
                    success: function (result) {
                        let event = result[0];
                        $('.row-event').html(`<div class="col-md-6 col-lg-4"><div class="card-box"><div class="card-thumbnail"><img src="http://localhost/friend-finder/public/img/${event.event_picture}" class="img-fluid" alt=""></div><div class="row mt-4"><div class="col-md-12"><span class="color-danger fw-semibold">${event.name}</span><h4 class="mt-1"><strong>${event.name_event}</strong></h4><span class="color-secondary mt-0 py-0">${event.event_owner}</span><ul class="list-unstyled mt-3"><li><div class="d-flex align-items-center"><i class="fas fa-tag me-2 color-primary"></i>IDR ${event.price}</div></li><li><div class="d-flex align-items-center"><i class="fas fa-map-marker-alt color-primary me-2"></i>${event.location}</div></li><li><div class="d-flex align-items-center"><i class="fas fa-calendar-minus me-2 color-primary"></i>${event.event_start_date}</div></li></ul></div></div><div class="row mt-3"><div class="col"><a href="<?= BASE_URL ?>/event/detail/<?= $event['id'] ?>" class="btn btn-primary btn-shadow">JOIN</a></div></div></div></div>`);
                    }
                });
            }
        }
